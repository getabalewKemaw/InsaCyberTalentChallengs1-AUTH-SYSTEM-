...
export function SignInForm({ onSuccess, onSwitchToSignUp }: SignInFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.error) {
        setError(res.error.message || "Failed to sign in. Please check credentials.");
      } else {
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="text-center">
        <h2 className="text-2xl font-bold text-[#554236]">Welcome Back</h2>
        <p className="text-sm text-[#594F4F] mt-1">Sign in to manage your active sessions</p>
      </div>
