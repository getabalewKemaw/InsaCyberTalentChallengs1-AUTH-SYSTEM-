          ...
          <div className="space-y-3">
            {revisions.length === 0 ? (
              ...
              revisions.map((rev, idx) => (
                <div
                  key={rev.id}
                  className="p-3 bg-white border border-brand-border rounded-lg shadow-sm hover:border-color-primary cursor-pointer transition-colors group"
                >
                  ...
                  <div className="flex justify-between items-center mt-2">
                    <span className="text-xs text-color-text-light">{formatDateTime(rev.createdAt)}</span>
                    {idx !== 0 && (
                      <button
                        onClick={() => restoreRevision(rev.id)}
                        className="opacity-0 group-hover:opacity-100 text-xs font-medium text-color-primary hover:underline transition-opacity"
                      >
                        Restore
                      </button>
                    )}
